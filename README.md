...
Версия 1.5: React UI на `:3000` + Express API и Socket.io на `:5050`.
Данные живут в SQLite, синхронизируются в реальном времени. Есть капсулы времени, общий бюджет, управление парой и локализация (8 языков + валюты).
...
Если пары ещё нет — после входа откроется мастер `/couple-setup` (создать пару или ввести код). Код приглашения всегда виден и копируется в **Настройки → Пара**. Ссылка вида `/register?code=TOGETHER1` подставит код после регистрации. Новый код инвалидирует старый.
...
Экраны: `/`, `/tasks`, `/calendar`, `/wishes`, `/capsules`, `/budget`, `/settings`.
...
- **i18next** + **react-i18next** — 8 языков интерфейса, валюта через `Intl.NumberFormat`
...
`/settings` — имя пары, дата встречи, код приглашения, **локализация (язык и валюта)**, профиль и выход из пары.

### 8. Локализация и валюты

Языки: `ru`, `en`, `es`, `fr`, `de`, `it`, `uk`, `pl`.
Валюты: RUB, USD, EUR, GBP, UAH, PLN. В MVP суммы **не** конвертируются — меняется символ и формат.

Приоритет языка: настройки пользователя → `localStorage` / язык браузера → геолокация по IP-заголовкам (`CF-IPCountry`, `Accept-Language`) → английский.

### Как добавить новый язык

1. Скопируйте `src/locales/en/translation.json` в `src/locales/XX/translation.json` (и в `public/locales/XX/` для документации).
2. Переведите все ключи. Не удаляйте вложенность `common` / `auth` / `dashboard` и т.д.
3. Добавьте импорт в `src/i18n/config.js` и код в `src/i18n/languages.js` (`LANGUAGES`).
4. Язык сам появится в **Настройки → Локализация** и на экранах входа.

Ключи i18n: `t('nav.tasks')`, `t('budget.addExpense')`. Новый экран — только новые ключи в JSON, без хардкода.
...
└── src/
    ...
    │   ├── config.js
    │   └── languages.js
    ├── locales/               # translation.json на 8 языках
    ├── App.jsx
    ├── index.css
    ├── components/
    │   ├── Header.jsx
